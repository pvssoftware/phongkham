...
def create_invoice(request, history_id):
    ...
    target_url = settings.INVOICE_SERVICE_HOST + '/e-invoices/create'
    ...
    invoice_payload['uu_id'] = uu_id
    print(f"Forwarding invoice creation request {uu_id} to {target_url} for company {company_id}")

    try:
        resp = requests.post(target_url, json=invoice_payload, headers=headers, timeout=30)
    ...
    if status_code == 200:
        data = resp_data["data"]
        invoice_uuid = data['uu_id']
        if invoice_uuid != uu_id:
